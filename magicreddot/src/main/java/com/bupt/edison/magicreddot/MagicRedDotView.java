...
public class MagicRedDotView extends View {
    ...
    MagicRedDotView magicRedDotViewInActivity; //Activity中的红点,window中的红点维持一个Activity中红点的引用
    MagicRedDotView magicRedDotViewInWindow; //Window中的红点,Acitvity中的红点需要维持一个Window中红点的引用
    ...
    public MagicRedDotView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public MagicRedDotView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public MagicRedDotView(Context context, WindowManager windowManager, int statusBarHeight,
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MagicRedDotView);
        //红点的半径
        dragDotRadius = typedArray.getDimensionPixelOffset(R.styleable.MagicRedDotView_dragDotRadius, 20);
        //锚点的半径
        anchorDotRadius = typedArray.getDimensionPixelOffset(R.styleable.MagicRedDotView_anchorDotRadius, 16);
        //红点的颜色
        dotColor = typedArray.getColor(R.styleable.MagicRedDotView_dotColor, Color.RED);
        //消息字体的颜色
        textColor = typedArray.getColor(R.styleable.MagicRedDotView_textColor, Color.WHITE);
        //消息字体的大小
        textSize = typedArray.getDimensionPixelSize(R.styleable.MagicRedDotView_textSize, 24);
        //红点的风格.0,实心点;1,QQ红点,可拖动;2,普通红点,不可拖动
        dotStyle = typedArray.getInt(R.styleable.MagicRedDotView_dotStyle, 2);
        //红点可拖动的距离.也是消失距离和复位距离;
        dragDistance = typedArray.getDimensionPixelOffset(R.styleable.MagicRedDotView_dragDistance, 150);
        //未读消息数的显示风格
        countStyle = typedArray.getInt(R.styleable.MagicRedDotView_countStyle, 1);
        //未读消息数的阈值
        msgThresholdValue = typedArray.getInt(R.styleable.MagicRedDotView_msgThresholdCount, 99);
        typedArray.recycle();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
            ...
            case MotionEvent.ACTION_DOWN:
                if (!isInitFromLayout) { //从代码中实例化View
                    ...
                    if (dragDotRectF.contains(downX, downY)) { //击中了红点,允许拖动
                        ...
                        magicRedDotViewInWindow = new MagicRedDotView.Builder().setAnchorDotRadius(this.anchorDotRadius)
                                ...
                                .create();
                        magicRedDotViewInWindow.setMagicRedDotViewInActivity(this);

                        addMagicRedDotViewToWindow(magicRedDotViewInWindow); //添加到window中
            ...
            case MotionEvent.ACTION_UP:
                if (!isInitFromLayout) {
                    if (isdragable && isInPullScale) { //在拉力范围内,要使红点复位
                        ...
                        //红点消失时的监听
                        if (null != getMagicRedDotViewInActivity().getOnDotDismissListener()) {
        ...
        //虽然magicRedDotViewInActivity会被隐藏,但是所有的Touch事件仍会被传递给
        //magicRedDotViewInActivity,所以需要在magicRedDotViewInActivity的onTouchEvent()函数中
        //把Touch事件传给magicRedDotViewInWindow
        if (magicRedDotViewInWindow != null) {
            magicRedDotViewInWindow.dispatchTouchEvent(event);
        }
        return true;
    }
    ...
    private void animationDismiss(float upX, float upY) {
        ...
        imageView.postDelayed(new Runnable() {
            ...
            public void run() {
                //更新未读消息数为0
                getMagicRedDotViewInActivity().setUnreadCount(0);

                animationDrawable.stop();
                imageView.clearAnimation();
                windowManager.removeView(imageView);
                removeMagicRedDotViewToWindow();
            }
        }, duration);
    }
    ...
    Animator.AnimatorListener animatorListener = new Animator.AnimatorListener() {
        ...
        public void onAnimationEnd(Animator animation) {
            resetStatus();
            getMagicRedDotViewInActivity().setVisibility(VISIBLE);
            getMagicRedDotViewInActivity().resetStatus();

            //红点复位时的监听
            if (null != getMagicRedDotViewInActivity().getOnDotResetListener()) {
                getMagicRedDotViewInActivity().getOnDotResetListener().OnDotReset();
            }
...
//        layoutParams.format = PixelFormat.TRANSLUCENT;
        ...
        layoutParams.x = 0;
        layoutParams.y = 0;
        windowManager.addView(magicRedDotView, layoutParams);
    }
    ...
    public static class Builder {
        ...
        public MagicRedDotView create() {
            return new MagicRedDotView(context, windowManager, statusBarHeight,
